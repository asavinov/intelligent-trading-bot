...
def generate_score2(df, ma):
    """
    Add a score column which aggregates different types of scores generated by various algorithms with different options.
    The score is added as a new column and is supposed to be used by the signal generator as the final feature.

    TODO: draft. needs to be re-worked. Originally intended for top-bot use case
    """

    df["top_nn"] = df["top10_2_nn"] + df["top9_2_nn"] + df["top8_2_nn"]
    df["top_score"] = df["top_nn"] / 3
    if ma:
        df["top_score"] = df["top_score"].rolling(window=ma, min_periods=1).apply(np.nanmean)

    df["bot_nn"] = df["bot10_2_nn"] + df["bot9_2_nn"] + df["bot8_2_nn"]
    df["bot_score"] = df["bot_nn"] / 3
    if ma:
        df["bot_score"] = df["bot_score"].rolling(window=ma, min_periods=1).apply(np.nanmean)

    # Final score: proportion to the sum
    high_and_low = df["top_score"] + df["bot_score"]
    df["score"] = ((df["top_score"] / high_and_low) * 2) - 1.0  # in [-1, +1]

    return df


def aggregate_scores(df, score_column, window: int, threshold: float) -> pd.Series:
    """
    Rolling aggregation of the specified score column using the specified aggregation function like
    (exponential) moving sum/mean or counts over point-wise threshold.

    TODO: draft. needs to be re-worked. Originally intended for top-bot use case
    """

    # If there is threshold, then convert to boolean column
    if threshold:
        score_sr = (df[score_column] >= threshold)
        score_sr = score_sr.rolling(window, min_periods=1).sum()
        score_sr = score_sr / window  # Portion of scores higher than threshold wrt window length
    else:
        score_sr = df[score_column]
        score_sr = score_sr.rolling(window, min_periods=1).mean()

    return score_sr


def score_to_signal(score_column, point_threshold, window, signal_threshold):
    """
    Given a point-wise score, return a boolean signal which can be used for decision making.
    First, apply point-wise threshold (optional), then moving average, and finally
    signal threshold.
    ...
    if isinstance(window, int):
        score_column = score_column.rolling(window).mean()
    elif isinstance(window, float):
        score_column = score_column.ewm(span=window, adjust=False).mean()

    # Signal
    score_column = score_column >= signal_threshold
...
def performance_score(df, sell_signal_column, buy_signal_column, price_column):
    ...
    The functions switchs the mode and searches for the very first signal of the opposite score.
